...
from core.vkernel_engine import VKernelEngine, build_canonical_graph
from core.io_encoders import DisturbanceEncoder

graph = build_canonical_graph()
encoder = DisturbanceEncoder(graph)
engine = VKernelEngine(graph)

x = encoder.radial_template()
...
x = encoder.ring_template()
x = encoder.add_noise(x, scale=0.08, seed=42)

result = engine.compute(x)
print("Dominant mode:", result.dominant_mode)

3 — symbolic / text-like mapping
token_map = {
    "flow": [1, 2, 3],
    "cycle": [7, 8, 9, 10, 11, 12],
    "event": [4, 13, 14],
...
x = encoder.from_text_tokens(["flow", "event"], token_map=token_map)
result = engine.compute(x)

print("Dominant mode:", result.dominant_mode)

4 — blended input
x1 = encoder.radial_template()
x2 = encoder.cluster_template()
x = encoder.blend(x1, x2, weights=[0.7, 0.3])
